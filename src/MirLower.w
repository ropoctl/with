...
fn MirBuilder.symbol_text(self: MirBuilder, sym: i32) -> str:
    let pool_text = self.pool.resolve_symbol(sym)
    if pool_text.len() > 0:
        return pool_text
    self.sema.pool_resolve(sym)

fn MirBuilder.symbols_match(self: MirBuilder, a: i32, b: i32) -> bool:
    if a == b:
        return true
    let a_text = self.symbol_text(a)
    a_text.len() > 0 and a_text == self.symbol_text(b)

fn MirBuilder.sema_symbol_for_ast_symbol(self: MirBuilder, sym: i32) -> i32:
    if sym == 0:
        return 0
...
fn MirBuilder.concrete_type(self: MirBuilder, type_id: i32) -> i32:
    if type_id <= 0:
        return type_id
...
fn MirBuilder.expr_type(self: MirBuilder, node: i32) -> i32:
    ...
    if self.sema.typed_expr_types.contains(node):
        ...
        if typed != 0:
            ...
            if typed == self.sema.ty_void as i32 and (node_kind == NodeKind.NK_IDENT or node_kind == NodeKind.NK_FIELD_ACCESS or node_kind == NodeKind.NK_BINARY):
    ...
    if type_name_sym != 0:
        ...
        if type_name == "Option":
            ...
            if method_name == "filter":
                return recv_type
            if method_name == "map" or method_name == "and_then" or method_name == "or_else" or method_name == "inspect" or method_name == "cloned":
                return recv_type
...
fn MirBuilder.tuple_index_from_field_token(self: MirBuilder, tuple_tid: i32, field_token: i32) -> i32:
    let resolved = self.sema.resolve_alias(tuple_tid)
...
fn MirBuilder.new_projected_field_place(self: MirBuilder, base: i32, field_token: i32, field_ty: i32) -> i32:
...
fn MirBuilder.operand_type(self: MirBuilder, operand_id: i32) -> i32:
    if operand_id < 0 or operand_id >= self.body.operand_kinds.len() as i32:
        return self.sema.ty_void as i32
...
fn MirBuilder.lower_binding_alias_place(self: MirBuilder, node: i32) -> i32:
    ...
    if kind == NodeKind.NK_FIELD_ACCESS:
        ...
        let field_sym = self.ast.get_data1(node)
        if self.sema.autoderef_step_counts.contains(base_expr):
            return self.new_projected_field_place(self.lower_recorded_autoderef_place(base_expr), field_sym, self.expr_type(node))
        ...
        while base_ty > 0:
            ...
            base_ty = self.sema.get_type_d0(resolved)
        return self.new_projected_field_place(field_base, field_sym, self.expr_type(node))
    -1
...
fn MirBuilder.generic_fn_node_for_sym(self: MirBuilder, sym: i32) -> i32:
...
fn MirBuilder.lower_method_call(self: MirBuilder, self_expr: i32, method_sym: i32, arg_start: i32, arg_count: i32, node: i32) -> i32:
    ...
    if callee_sym == method_sym or self.sym_is_generic_fn(callee_sym):
...
fn MirBuilder.tuple_operand_from_fields(self: MirBuilder, fields: Vec[i32], result_ty: i32, span: i32) -> i32:
    let names: Vec[i32] = Vec.new()
    for _ in 0..fields.len() as i32:
        names.push(0)
    let fid = self.body.new_agg_fields(fields, names)
...
fn MirBuilder.clone_or_copy_place(self: MirBuilder, payload_place: i32, payload_ty: i32, node: i32) -> i32:
    ...
    let clone_fn = if type_sym != 0: self.sema.lookup_method_fn(type_sym, clone_sym) else: 0
    if clone_fn == 0:
        self.mark_unsupported()
        return self.operand_for_place(payload_place, payload_ty)
    let sig_idx = self.sema.lookup_method_sig(type_sym, clone_sym)
    let expected = if sig_idx >= 0 and self.sema.sig_get_param_count(sig_idx) > 0: self.sema.sig_param_type(sig_idx, 0) else: 0
    let args: Vec[i32] = Vec.new()
    args.push(self.operand_for_place_arg(payload_place, payload_ty, expected, self.ast.get_start(node)))
    self.lower_resolved_call_with_operand_args(clone_fn, args, payload_ty, node)

fn MirBuilder.lower_option_flatten_method(self: MirBuilder, self_expr: i32, arg_count: i32, node: i32) -> i32:
    if arg_count != 0:
        self.mark_unsupported()
        return self.unit_operand()
    let span = self.ast.get_start(node)
    ...
    self.assign_operand_to_place(result_place, self.operand_for_place(inner_place, inner_option_ty), span)
    self.terminate(TermKind.TK_GOTO, join_bb, 0, 0, 0)
...
fn MirBuilder.lower_result_ok_err_method(self: MirBuilder, self_expr: i32, method_name: str, arg_count: i32, node: i32) -> i32:
    if arg_count != 0:
        self.mark_unsupported()
        return self.unit_operand()
    let span = self.ast.get_start(node)
...
fn MirBuilder.lower_option_zip_method(self: MirBuilder, self_expr: i32, arg_start: i32, arg_count: i32, node: i32) -> i32:
    if arg_count != 1:
        self.mark_unsupported()
        return self.unit_operand()
    let span = self.ast.get_start(node)
...
fn MirBuilder.lower_option_unzip_method(self: MirBuilder, self_expr: i32, arg_count: i32, node: i32) -> i32:
    if arg_count != 0:
        self.mark_unsupported()
        return self.unit_operand()
    let span = self.ast.get_start(node)
...
fn MirBuilder.lower_option_combinator_method(self: MirBuilder, self_expr: i32, method_name: str, arg_start: i32, arg_count: i32, node: i32) -> i32:
    ...
    if method_name == "and_then":
        ...
        else if method_name == "cloned":
            payload_op = self.clone_or_copy_place(payload_place, payload_ty, node)
        else:
            payload_op = self.operand_for_place(payload_place, payload_ty)
        let some_fields: Vec[i32] = Vec.new()
...
fn MirBuilder.lower_unwrap_or_else_method(self: MirBuilder, self_expr: i32, arg_start: i32, arg_count: i32, node: i32) -> i32:
    if arg_count != 1:
        self.mark_unsupported()
        return self.unit_operand()
    var value_ty = self.expr_type(self_expr)
    ...
    let success_payload_place = self.body.new_field_place(success_downcast, 0, result_ty)
    self.assign_operand_to_place(result_place, self.operand_for_place(success_payload_place, result_ty), span)
    self.terminate(TermKind.TK_GOTO, join_bb, 0, 0, 0)
    ...
    let default_op = self.lower_call_with_operand_args(fallback_op, call_args, result_ty, node)
...
fn MirBuilder.lower_implicit_default_value(self: MirBuilder, type_id: i32, span: i32) -> i32:
    ...
    if not ret_is_void and builder.sema.get_type_kind(ret_resolved) == TypeKind.TY_GENERIC_INST:
        ...
        if builder.sema.pool_resolve(ret_base) == "Result" and builder.sema.get_generic_inst_arg_count(ret_resolved) == 2:
            ...
            if result_body_ty != 0 and result_body_ty != ret_ty:
                if builder.sema.types_compatible(ok_type, result_body_ty) != 0 or builder.sema.arithmetic_result_type(ok_type, result_body_ty) != 0:
